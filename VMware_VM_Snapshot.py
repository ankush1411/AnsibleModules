...
def take_snapshot(hostname,vcenter,username,password):
    print("starting")
        ...
        for vm in container.view:
            ...
            if hasattr(vm.guest, 'hostName') and vm.guest.hostName:
               if hostname in vm.guest.hostName:
                  ...
                  if free_space - size > 500:
                      print("trying to create snapshot")
                      vm.CreateSnapshot_Task(name='test_snap',description='test',memory=False,quiesce=False)
...
def run_module():
    ...
    try:
       res = take_snapshot(module.params['hostname'],module.params['vcenter'],module.params['username'],module.params['password'])
       if res == "successful":
